...
# Start the demo app
java -classpath target/amazon-kinesis-video-streams-producer-sdk-java-1.9.5-jar-with-dependencies.jar:$classpath_values -Daws.accessKeyId=${ACCESS_KEY} -Daws.secretKey=${SECRET_KEY} -Dkvs-stream=${KVS_STREAM} -Djava.library.path=/opt/amazon-kinesis-video-streams-producer-sdk-cpp/kinesis-video-native-build com.amazonaws.kinesisvideo.demoapp.DemoAppMain
