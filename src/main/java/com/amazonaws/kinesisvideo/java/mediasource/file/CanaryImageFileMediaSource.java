 ...
 */
public class CanaryImageFileMediaSource implements MediaSource {
    ...
    private CanaryImageFileMediaSourceConfiguration canaryImageFileMediaSourceConfiguration;
    private MediaSourceState mediaSourceState;
    private MediaSourceSink mediaSourceSink;
    private CanaryImageFrameSource canaryImageFrameSource;

    public CanaryImageFileMediaSource(@Nonnull final String streamName) {
        this(streamName, new CompletableFuture<>());
    }

    public CanaryImageFileMediaSource(@Nonnull final String streamName, final CompletableFuture<Boolean> future) {
    ...
    public MediaSourceConfiguration getConfiguration() {
        return canaryImageFileMediaSourceConfiguration;
    }
    ...
    public StreamInfo getStreamInfo() throws KinesisVideoException {
        return new StreamInfo(VERSION_TWO,
                ...
                canaryImageFileMediaSourceConfiguration.getContentType(),
                NO_KMS_KEY_ID,
                RETENTION_ONE_HOUR,
                NOT_ADAPTIVE,
                MAX_LATENCY_ZERO,
                ...
                RECOVER_ON_FAILURE,
                codecIdFromContentType(canaryImageFileMediaSourceConfiguration.getContentType()),
                "test-track",
                DEFAULT_BITRATE,
                canaryImageFileMediaSourceConfiguration.getFps(),
    ...
    public void configure(final MediaSourceConfiguration configuration) {
        Preconditions.checkState(this.canaryImageFileMediaSourceConfiguration == null);

        if (!(configuration instanceof CanaryImageFileMediaSourceConfiguration)) {
            throw new IllegalStateException("Configuration must be an instance of CanaryImageFileMediaSourceConfiguration");
        }

        this.canaryImageFileMediaSourceConfiguration = (CanaryImageFileMediaSourceConfiguration) configuration;
    }
    ...
    public void start() throws KinesisVideoException {
        ...
        canaryImageFrameSource = new CanaryImageFrameSource(canaryImageFileMediaSourceConfiguration);
        canaryImageFrameSource.onStreamDataAvailable(new DefaultOnStreamDataAvailable(mediaSourceSink));
        canaryImageFrameSource.start();
    }

    @Override
    public void stop() throws KinesisVideoException {
        if (canaryImageFrameSource != null) {
            canaryImageFrameSource.stop();
        }
