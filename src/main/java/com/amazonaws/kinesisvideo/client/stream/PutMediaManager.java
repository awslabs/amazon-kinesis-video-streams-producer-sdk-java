...
public class PutMediaManager {
    ...
    public void sendTestMkvStream(final ClientConfiguration config) throws Exception {
        ...
        PutMediaClient.builder()
                .putMediaDestinationUri(config.getStreamUri())
                .mkvStream(testMkvStream)
                .streamName(config.getStreamName())
                .timestamp(streamStartTime)
                .fragmentTimecodeType(ABSOLUTE) //TODO: Add as a cmd line parameter.
                .signWith(signer)
                .receiveAcks(new StreamConsumer(config.getApiName()))
                .build()
                .putMediaInBackground();
    }

    public void sendMkvStreamWithAbsoluteFragmentTimeCode(final ClientConfiguration config,
            final InputStream inputStream, final Consumer<InputStream> consumer,
            final Integer receiveTimeout) throws Exception {
            PutMediaClient.builder()
                .putMediaDestinationUri(config.getStreamUri())
