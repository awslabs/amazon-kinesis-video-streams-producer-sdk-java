    ...
    */
    private StreamEventMetadata(String imagePrefix, int numberOfPairs, String[] names, String[] values) {
        ...
        if (numberOfPairs > 0) {
            ...
            if (names.length != values.length || names.length != numberOfPairs) {
                throw new IllegalArgumentException("The length of the arrays do not match: names " + names.length + ", values: " + values.length + ",  numberOfPairs: " + numberOfPairs);
            }
